...
// Because the url is always `https://api.k00.top/version/latest`.
#[tokio::main(flavor = "current_thread")]
...
fn get_api_server_(api: String, custom: String) -> String {
    ...
    "http://k00.top:21114".to_owned()
}

#[inline]
pub fn is_public(url: &str) -> bool {
    let url = url.to_ascii_lowercase();
    url.contains("k00.top/") || url.ends_with("k00.top")
}
...
mod tests {
    ...
    #[test]
    fn test_is_public() {
        // Test URLs containing "k00.top/"
        assert!(is_public("http://k00.top/"));
        assert!(is_public("https://www.k00.top/"));
        assert!(is_public("https://api.k00.top/v1"));
        assert!(is_public("https://API.k00.top/v1"));
        assert!(is_public("http://k00.top/path"));

        // Test URLs ending with "k00.top"
        assert!(is_public("k00.top"));
        assert!(is_public("http://k00.top"));
        assert!(is_public("http://k00.top"));
        assert!(is_public("http://www.k00.top"));
        assert!(is_public("https://api.k00.top"));
        ...
        assert!(!is_public("http://k00.topputer.com"));
        assert!(!is_public("k00.tophello.com"));
    }
